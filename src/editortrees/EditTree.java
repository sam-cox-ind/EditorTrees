...
public class EditTree {
	...
	public String toString() {
		if (root == Node.NULL_NODE)
			return "";
		return root.left.toString() + root.data + root.right.toString(); // replace with a real calculation.
	}
...
//			root = root.add(ch, info);
//			this.numRotations += info.numOfRotations;
			boolean[] bc = new boolean[1];
			bc[0] = true;
			root = root.add(ch,totalRotation,bc);
		}
		this.size++;
	}
...
//			this.numRotations += info.numOfRotations;
			boolean[] bc = new boolean[1];
			bc[0] = true;
			root = root.add(ch, pos,totalRotation,bc);
			this.size++;
		}
	}
	...
	public String toRankString() {
		String re = "[";
		String temp = root.toRankString();
		if (temp.length() > 0)
			temp = temp.substring(0,temp.length() - 2);
		re = re + temp;
		re = re + "]";
		return re; // replace by a real calculation.
	}
	...
	public char get(int pos) throws IndexOutOfBoundsException {
		if (pos > this.size - 1 || pos < 0 || root == Node.NULL_NODE) {
			throw new IndexOutOfBoundsException();
		}
		else {
	...
	public String toDebugString() {
	    ...
	    String rootDebugStr = root.toDebugString();
	    
	    if (!rootDebugStr.isEmpty()) {
	        rootDebugStr = rootDebugStr.substring(0, rootDebugStr.length() - 2);
	        debugStr.append(rootDebugStr);
	    }
...
//		return this.numRotations; // replace by a real calculation.
	}
	...
	public char delete(int pos) throws IndexOutOfBoundsException {
		...
		// *successor*.
		if( pos >= this.size || pos < 0 || root == Node.NULL_NODE) {
			throw new IndexOutOfBoundsException();
		}else {
			Node.Container container = new Node.Container();
			root = root.delete(pos,container,totalRotation);
			return container.re; // replace by a real calculation.
		}
	}
	...
	public String get(int pos, int length) throws IndexOutOfBoundsException {
		if (pos + length > this.size || pos < 0 || root == Node.NULL_NODE) {
			throw new IndexOutOfBoundsException();
		} else {
			StringBuilder re = new StringBuilder(length);
			root.get(root.rank, pos, pos + length, re);
			return re.toString();
	...
	//show method that helps with graphical debugger
	public void show() {
	 ...
	 * closes the tree window, still keeps all the data, 
	   and you can still re show the tree with the show() method
	 */
	public void close() {
		if (this.display != null) {
			this.display.close();
...
//		
//		int numOfRotations;
//		ArrayList<String> nodeTrail;
//		
//		public NodeInfo() {
//			this.numOfRotations = 0;
//			this.nodeTrail = new ArrayList<String>();
//		}
//		
//	}
}
